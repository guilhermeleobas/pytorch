...
// Frame hooks now store the C function pointer directly (no PyCapsule); the
// interpreter keeps a linked list of hook entries. Register/enable/disable by
// the raw function pointer via the *FrameHookFunction API.
void enable_frame_hook_shim(PyThreadState* tstate) {
  DEBUG_TRACE0("enable_frame_hook_shim entered");
  if (PyUnstable_ContainsFrameHookFunction(tstate->interp, hook_function)) {
    DEBUG_TRACE0("contains hook function, enabling...");
    PyUnstable_EnableFrameHookFunction(tstate->interp, hook_function);
  } else {
    PyUnstable_AddFrameHookFunction(tstate->interp, hook_function);
  }
}
...
void clear_frame_hook_shim(PyThreadState* tstate) {
  if (PyUnstable_ContainsFrameHookFunction(tstate->interp, hook_function)) {
    ...
    PyUnstable_DisableFrameHookFunction(tstate->interp, hook_function);
