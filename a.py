...
from torch.func import grad, jvp, vjp, vmap, linearize
from functorch.dim import dims
from torch._functorch.vmap import _process_batched_inputs
...
class TwoTensor(torch.Tensor):
    ...
    def __new__(cls, a, b, outer_size=None, outer_stride=None):
        if outer_size == None:
            outer_size = [a.shape[0] * 2, a.shape[1]]
            # outer_size = a.size()
...
def fn(x):
    ...
    # return x.sin()
    return x * x.shape[0]
    # return x.view(x.shape[0], x.shape[1]) * 2
